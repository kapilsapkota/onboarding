...
@php
    $base = 'flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition focus:outline-none group';
    $activeClass = 'bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400';
    $inactiveClass = 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white';
    ...
    $icons = [
        ...
        'smtp'       => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 12h14M12 5l7 7-7 7"/>',
        ...
        'campaign'   => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z"/>',
        ...
        'clients' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 21h18M9 8h6M9 12h6M9 16h6M4 21V7a2 2 0 012-2h12a2 2 0 012 2v14"/>',
    ];

    $svgPath = $icons[$icon] ?? $icons['dashboard'];
@endphp

<a {{ $attributes->merge(['class' => "$base " . ($active ? $activeClass : $inactiveClass)]) }}>
    <svg class="h-5 w-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" :title="collapsed ? '{{ $slot }}' : ''">
        {!! $svgPath !!}
    </svg>
