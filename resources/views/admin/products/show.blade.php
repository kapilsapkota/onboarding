    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
                ...
                @if((int)$product->quote_default === 1)
                    <span class="relative group inline-flex">
                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300 cursor-help">
                    Quote Default
                </span>

                <span class="absolute top-full left-1/2 -translate-x-1/2 mt-2 hidden group-hover:block w-52 rounded-md bg-gray-900 px-3 py-2 text-xs text-white shadow-lg z-50">
                    Automatically included when creating a new quote.
                </span>
            </span>
                @endif
            </div>
            ...
            <div class="flex items-center gap-2">
                <a href="{{ route('admin.products.edit', $product) }}"
                   class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-200 text-sm font-medium rounded-lg hover:bg-gray-50 dark:hover:bg-gray-600 transition">
                    Edit
                </a>
                ...
                <form method="POST" action="{{ route('admin.products.destroy', $product) }}"
                    ...
                    <button type="submit"
                            class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 text-red-500 text-sm font-medium rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 transition">
                        Delete
                    </button>
                </form>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white dark:bg-gray-800 shadow-sm rounded-xl p-6">
                    <div class="flex items-start gap-4">
                        ...
                        @else
                            <div
                                class="w-16 h-16 rounded-xl bg-gray-100 dark:bg-gray-700 flex items-center justify-center flex-shrink-0">
                        ...
                        <div>
                            ...
                            @if($product->category)
                                <span
                                    class="mt-1 inline-block text-xs text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 rounded px-2 py-0.5">
                        ...
                        <dl class="space-y-3">
                            <div class="flex items-center justify-between">
                                <dd>
                                    ...
                                    @else
                                        <div
                                            class="rounded-xl bg-gray-100 dark:bg-gray-700 flex items-center justify-center flex-shrink-0">
                                            <svg class="w-7 h-7 text-gray-400" fill="none" stroke="currentColor"
                                                 viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                      d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10"/>
                    ...
                    <dl class="space-y-3">
                        <div class="flex items-center justify-between">
                            ...
                            <dd>
                                ...
                                @endphp
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $typeStyles[$product->price_type] ?? '' }}">
                        ...
                        @if($product->price_type === 'fixed')
                            <div class="flex items-center justify-between">
                                ...
                                <dd class="text-sm font-semibold text-gray-900 dark:text-gray-100">
                                    ${{ number_format($product->fixed_price, 2) }}</dd>
                            </div>
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <dd class="text-sm font-semibold text-gray-900 dark:text-gray-100">
                                    @if($product->price_min !== null && $product->price_max !== null)
                                        ...
                                        ${{ number_format($product->price_max, 2) }}
                                    @endif
                                </dd>
                            </div>
                            <div class="flex items-center justify-between">
                                ...
                                <dd class="text-sm text-gray-700 dark:text-gray-300">
                                    {{ $product->price_increment !== null ? '$' . number_format($product->price_increment, 2) : '' }}
                                </dd>
                            </div>
                            <div>
                                ...
                                <dd class="flex flex-wrap gap-1.5">
                                    @foreach($product->getPriceOptions() as $opt)
                                        <span
                                            class="px-2 py-0.5 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded text-xs font-mono">
                                            ${{ number_format($opt, 2) }}
                                        </span>
                                    @endforeach
                                </dd>
                        ...
                        @elseif($product->price_type === 'hourly')
                            <div class="flex items-center justify-between">
                                ...
                                <dd class="text-sm font-semibold text-gray-900 dark:text-gray-100">
                                    {{ $product->hourly_rate !== null ? '$' . number_format($product->hourly_rate, 2) . '/hr' : '' }}
                                </dd>
                            </div>
                        @endif
